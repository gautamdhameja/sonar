...
test("extractDependencyEdges resolves TypeScript source for .js import specifiers", () => {
  ...
  assert.deepEqual(
    edges.map((edge) => `${edge.sourceFile}->${edge.targetFile}`),
...
test("extractDependencyEdges resolves directory index and export-from imports", () => {
  const edges = extractDependencyEdges([
    unit("main", "src/main.ts", ["export { run } from './framework/pipeline/index.js';", "import './setup.js';"]),
    unit("index", "src/framework/pipeline/index.ts", []),
    unit("setup", "src/setup.ts", []),
  ]);

  assert.deepEqual(
    new Set(edges.map((edge) => edge.targetFile)),
    new Set(["src/framework/pipeline/index.ts", "src/setup.ts"]),
  );
});
...
test("extractDependencyEdges resolves common src aliases", () => {
  ...
  assert.deepEqual(
    new Set(edges.map((edge) => edge.targetFile)),
    new Set(["src/llama/config.ts", "src/framework/pipeline/runner.ts"]),
  );
});
