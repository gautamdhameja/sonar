...
A briefing session stores:

- The generated codebase briefing.
...
Follow-up questions use this session state to improve retrieval. Sonar boosts files already cited in the briefing, classifies the follow-up intent, performs exact and lexical search for named files or concepts, uses vector retrieval when useful, and expands through the dependency graph for workflow questions.
...
The briefing and prior messages are used for orientation. Concrete implementation claims should come from the retrieved source context.
...
- `summary_graph`: architecture and briefing questions include the stored codebase summary and pack a compact set of supporting code.
...
The dedicated briefing flow adds another retrieval pass that favors product documentation, app/package boundaries, entry points, collaboration/sharing files, local persistence files, and privacy or operational evidence. Follow-up retrieval pins smaller relevant units from previously cited files so the session stays grounded in the material the user already saw.
